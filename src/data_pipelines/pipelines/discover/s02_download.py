...
# Bounds how many lessons are "in flight" (and therefore have a visible progress row)
# at once. Not a rate limit — that's each adapter's own job (e.g. YouTubePlaylist
# Adapter's semaphore=1) — this is purely about keeping the live display readable and
# not scheduling thousands of asyncio tasks at once when a whole series is queued.
MAX_CONCURRENT = 5
...
def run_downloads(
    ...
    print(f"{len(jobs)} lesson(s) to download")
