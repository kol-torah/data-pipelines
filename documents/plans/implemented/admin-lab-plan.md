...
**Status: implemented.** All four phases are built and running: the `lab` schema
migration (`alembic/versions/a12032085f36_*.py`), the FastAPI backend
(`src/data_pipelines/admin_lab_api/`), the React frontend (`frontend/`), the job
framework and the transcribe/diarize jobs (`src/data_pipelines/lab/`), and the synced
results view. `admin_app.py` and the `streamlit` dependency are gone, per §3.4. Kept
here for historical context only — `documents/admin-lab.md` and `documents/design.md`
are the durable references; this file is no longer updated as the code evolves.

**Code that was touched:** new `frontend/` (React), new `src/data_pipelines/lab/`, new
`src/data_pipelines/admin_lab_api/`; retired `src/data_pipelines/admin_app.py`; touched
...
| Phase | Delivers | New runtime deps | Status |
| --- | --- | --- | --- |
| 1. Infrastructure | `lab` schema + `lab_jobs` migration; FastAPI skeleton; React/Vite/Tailwind skeleton; dev workflow | `fastapi`, `uvicorn`; frontend toolchain | **done** |
| 2. Catalogue admin | Rabbi/series/lesson CRUD, drill-down, lesson status, Reset All Lessons — retires `admin_app.py` | none | **done** |
| 3. Job execution | `LabJob` framework, lesson picker + auto-download, transcribe/diarize jobs, run/status API | `torch`, `transformers`, `pyannote.audio`, `soundfile` | **done** |
| 4. Results view | Range-request audio streaming; synced virtualized transcript/diarization lists; host heuristic; log view | none (frontend-only beyond phase 1's deps) | **done** |
